...
var process_metrics = function (metrics, timer_metrics_filters, flushInterval, ts, flushCallback) {
    ...
    for (key in timers) {
      ...
      timer_data[key] = filtered_timer_metrics(current_timer_data, timer_metrics_filters);
    }
...
var filtered_timer_metrics = function (timer_metrics, timer_metrics_filters = []) {
  if (!Array.isArray(timer_metrics_filters) || timer_metrics_filters.length == 0) {
    ...
    return Object.keys(timer_metrics)
      .filter((key) => {
        ...
        return timer_metrics_filters.includes(cleaned_key);
      })
      .reduce((obj, key) => {
