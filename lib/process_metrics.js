...
var process_metrics = function (metrics, filters, flushInterval, ts, flushCallback) {
    ...
    for (key in timers) {
      ...
      timer_data[key] = filtered_timer_metrics(current_timer_data, filters);
    }
...
var filtered_timer_metrics = function (timer_metrics, filters = []) {
  return Object.keys(timer_metrics)
    .filter((key) => {
      ...
      let cleaned_key = /\d/.test(key) ? [...key.split("_").slice(0, -1), "percent"].join("_") : key
      return !filters.includes(cleaned_key);
    })
    .reduce((obj, key) => {
