...
var post_stats = function graphite_post_stats(statString) {
  ...
  if (graphiteHostDup) {
    try {
      var graphite = net.createConnection(graphitePortDup, graphiteHostDup);
...
exports.init = function graphite_init(startup_time, config, events, logger) {
  debug = config.debug;
  l = logger;
  graphiteHostDup = config.graphiteHostDup;
  graphitePortDup = config.graphitePortDup;
